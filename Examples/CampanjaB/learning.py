...
def mapper_regression_armar(line):
  '''
  mapper function for armar model
  '''
  data = np.array([ [item[0],item[1]] for item in line[1] ])
  data = data[data[:,0].argsort()]
  data = pd.DataFrame(data[:,1])
  data.index = pd.Index(sm.tsa.datetools.dates_from_range('2001', '2006'))
  #pBest,qBest,rmseBest = 0,0,100000000
  #for p,q in itertools.product(range(1,10),range(1,10)):
  #  res = np.array(sm.tsa.ARMA(data, (1,0)).fit().predict('2001','2007'))
  #  rmse = math.sqrt(np.sum(np.array([x**2 for x in res[0:6]-np.array(data)])))
  #  if rmse<rmseBest:
  #    rmseBest,pBest,qBest = rmse,p,q
  try:
    res = np.array(sm.tsa.ARMA(data, (2,0)).fit().predict('2001','2007'))
    rmse = math.sqrt(np.sum(np.array([x**2 for x in res[0:6]-np.array(data)])))
    return (line[0],(res[6], rmse))
  except:
    return (line[0],(0,0))

  pass

def local_autoregression(data,name):
  '''
  autoregressive moving average
  '''
  logging.warning('--------------------------------- regression: %s --------------------------------- ' % name)
  #data = data.filter(lambda x: x[0]<10).map(mapper_regression_armar)
  data = data.map(mapper_regression_armar)
  return data
  pass
